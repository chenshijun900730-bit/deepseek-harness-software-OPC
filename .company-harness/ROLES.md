...
> 模型与 reasoning 硬约束（H-02）：Coordinator 是唯一使用 max 的角色；其余全部为 high。`company_record_role` 对每次启动做硬校验。
...
| 8 | final-evaluator | 最终验收负责人 | deepseek-v4-pro | high | 在干净环境完成跨 Sprint 端到端验收 | 不参与任何 Sprint 实现 | 中型+最终验收 |
| 9 | security-reviewer | 安全/数据迁移评审 | deepseek-v4-pro | high | 权限、支付、隐私、删除、迁移专项检查 | 不以普通功能通过替代安全通过 | 高风险强制 |
| 10 | explorer | Explorer 调查员 | deepseek-v4-flash | high | 仓库、官方资料、依赖、测试入口和风险调查 | 只读；不修改代码 | 中型及以上 |
| 11 | qa-runner | QA 执行员 | deepseek-v4-flash | high | UI、API、数据库、构建和回归测试；采集证据 | 不判最终结论；不修业务代码 | 中型及以上 |
| 12 | mechanical-worker | Mechanical Worker | deepseek-v4-flash | high | 明确、重复、可批量验证的机械任务 | 不处理歧义、架构或跨模块决策 | 按需 |
| 13 | recorder | Recorder 项目秘书 | deepseek-v4-flash | high | 状态、用量、证据索引、HANDOFF 和项目收据 | 只记录事实；不做产品或技术决策 | 中型及以上 |
...
小型 1 个；中型最多 3 个；复杂与高风险最多 4 个。高风险评审与编码不得在同一未冻结代码状态下并发。
